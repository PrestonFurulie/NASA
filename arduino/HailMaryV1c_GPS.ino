...
CHANGELOG V1c - GPS Integration
- Added TinyGPS++ library for GPS parsing
- Added GPS UTC timestamp logging
- Added GPS coordinates (lat/lon) to data
- Serial3 used for GPS module communication (9600 baud)
- GPS provides accurate timestamps for time-series graphing
- Graceful fallback: continues logging even without GPS fix
...
- GPS provides UTC timestamps and coordinates for accurate time-series analysis.
...
TinyGPSPlus gps;  // GPS parser object (uses Serial3)
...
struct DataPoint {
  ...
  int utcYear, utcMonth, utcDay;        // GPS UTC date
  int utcHour, utcMinute, utcSecond;    // GPS UTC time
...
bool gpsWorking = true;        // GPS module flag
...
void setup() {
  ...
  Serial3.begin(9600);         // GPS module communication (9600 baud)
  ...
  if (gpsWorking) {
    ...
    Serial.println("GPS not working, skipping test");
  }
...
void loop() {
  ...
  if (currentTime - lastWriteTime >= 2000) { // Less frequent than data logging
   ...
   if ((currentTime / 10000) * 10000 == currentTime) { // Every 10 seconds
      ...
      if (gpsWorking) gpsSerialTest();  // NEW
      // bmpSerialTest();
      // UVSerialTest();
      Serial.println("----------------------------\n");
    }
...
/*FUNCTIONS < JUMP-TO KEY >
    ...
    19) gpsTestnSetup    // GPS module initialization
    20) getGPSdata       // Read and parse GPS data
    21) gpsSerialTest    // GPS debugging output
  */
/////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
...
void writeHeader() {  // Write header(creates and writes header of csv file collumns)
  ...
  for (int attempt = 0; attempt < 3 && !isWritten; attempt++) {
    ...
    if (dataFile) {
      dataFile.println("UTC_Date,UTC_Time,GPS_Fix,elapsed(ms),UV_value1A,UV_value1B,UV_value1C,UV_value2A,UV_value2B,UV_value2C,UV_value3A,UV_value3B,UV_value3C, UV_value4A,UV_value4B,UV_value4C, pressure_bmp390, temp_bmp390, alt_bmp390, gyro_X, gyro_Y, gyro_Z, accel_X, accel_Y, accel_Z");
...
void logData() {
  ...
  // Get GPS data for UTC timestamp
  if (getGPSdata()) {
    newDataPoint.utcYear = gpsYear;
    newDataPoint.utcMonth = gpsMonth;
    newDataPoint.utcDay = gpsDay;
    newDataPoint.utcHour = gpsHour;
    newDataPoint.utcMinute = gpsMinute;
    newDataPoint.utcSecond = gpsSecond;
    newDataPoint.gpsFix = gpsHasFix;
  } else {
    newDataPoint.gpsFix = false;
    // Use zeros to indicate no GPS data
    newDataPoint.utcYear = 0;
    newDataPoint.utcMonth = 0;
    newDataPoint.utcDay = 0;
    newDataPoint.utcHour = 0;
    newDataPoint.utcMinute = 0;
    newDataPoint.utcSecond = 0;
  }
...
void writeDataPointToFile(DataPoint data, File file) {
  // Write UTC timestamp from GPS
  if (data.gpsFix && data.utcYear > 0) {
    file.printf("%04d-%02d-%02d,", data.utcYear, data.utcMonth, data.utcDay);
    file.printf("%02d:%02d:%02d,", data.utcHour, data.utcMinute, data.utcSecond);
    file.print("1,");  // GPS fix = true
  } else {
    file.print("NO_GPS,NO_GPS,0,");  // No GPS fix
  }
...
////////////////////// FUNCTION_19: gpsTestnSetup ///////////////////////
void gpsTestnSetup() {
  Serial.println("GPS Module Test Begin");
  
  // Wait up to 30 seconds for GPS data on Serial3
  unsigned long startTime = millis();
  bool gpsDataReceived = false;
  
  Serial.println("Waiting for GPS data... (30 second timeout)");
  
  while ((millis() - startTime) < 30000 && !gpsDataReceived) {
  ...
  if (gpsDataReceived) {
    ...
    if (gps.location.isValid()) {
      ...
      Serial.println("GPS module responding but no fix yet");
      gpsHasFix = false;
    }
  } else {
    Serial.println("GPS Module not responding - check wiring!");
    Serial.println("GPS TX should connect to Arduino Mega Pin 15 (RX3)");
    Serial.println("GPS RX should connect to Arduino Mega Pin 14 (TX3)");
    gpsWorking = false;
  }
}

////////////////////// FUNCTION_20: getGPSdata ///////////////////////
boolean getGPSdata() {
  if (!gpsWorking) {
    return false;
  }
...
////////////////////// FUNCTION_21: gpsSerialTest ///////////////////////
void gpsSerialTest() {
  if (!gpsWorking) {
    Serial.println("GPS sensor not working");
    return;
  }
  ...
  if (gpsHasFix && gpsYear > 0) {
    // Print UTC timestamp: "2025-10-17 14:23:45"
    ...
    Serial.println("UTC Time: NO_GPS");
    Serial.println("GPS Coordinates: NO_GPS, NO_GPS");
    Serial.println("Satellites: 0");
    Serial.println("GPS Fix: NO_FIX");
  }
